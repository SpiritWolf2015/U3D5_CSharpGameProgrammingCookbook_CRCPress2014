...
public class Waypoints_Controller : MonoBehaviour {
    ...
    public void GetTransforms ( ) {
        ...
        // which is initialised here (we always need to do this 
    ...
    // same transform as one passed in. We use this to make sure that 
    // when we are looking for the nearest waypoint we don’t find the 
    // same one as we just passed
    public int FindNearestWaypoint ( Vector3 fromPos, Transform exceptThis, float maxRange ) {
        ...
        for ( int i = 0; i < transforms.Count; i++ ) {
            ...
            // calculate the distance between the curren
            // transform and the passed in transform’s 
            // position vector
            diff = (TEMPtrans.position - fromPos);
            curDistance = diff.sqrMagnitude;
    ...
    public Transform GetWaypoint ( int index ) {
        ...
        // make sure that we have populated the transforms list, i
        // not, populate it
        if ( transforms == null )
            GetTransforms( );
