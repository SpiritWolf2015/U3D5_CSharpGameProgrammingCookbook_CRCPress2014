...
public class BaseAIController : ExtendedCustomMonoBehavior {
    ...
    public void TurnTowardTarget ( Transform aTarget ) {
        ...
        relativeTarget = rotateTransform.InverseTransformPoint(aTarget.position);  // note we use rotateTransform as a 
    ...
    public bool CanSee ( Transform aTarget ) {
        ...
        if ( Physics.Raycast(myTransform.position + (visionHeightOffset * myTransform.up), tempDirVec, out hit, maxChaseDistance) ) {
            ...
            if ( hit.transform.gameObject.layer == aTarget.gameObject.layer ) {
                return true;
            }
        }
    ...
    void UpdateWaypoints ( ) {
        ...
        // initialised, it is possible for this function to be called
        // before we have actually finished running the waypoints
